...
void menu(Escaninho *pessoa, int tam){
    ...
    do{
        ...
        scanf("%d", &op);
        switch(op){
            case 1: leitura(pessoa, tam);break;//chamada da função para cadastro de pessoa
            case 2: buscpfacomp(pessoa, tam);break;//busca de volumes por cpf do acompanhante
            case 3: buscacpfpaci(pessoa, tam);break;//busca de volumes por cpf do paciente
            case 4: volumetotal(pessoa, tam);break;//relatório dos volumes totais contendo: cpfpaci, cpfacomp e localizacao;
            case 5: volumeportipo(pessoa, tam);break;
            case 6: volumeporlocali(pessoa, tam);break;
            case 7: printf("\nSaindo...");break;
            default: printf("\nOpção inexistente");break;
        }
    }while(op!=7);
